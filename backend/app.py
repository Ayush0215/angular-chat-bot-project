...
async def login(user: UserLogin):
    try:
        ...
        pre_hashed_pw = hashlib.sha256(user.password.encode('utf-8')).hexdigest()

        # Verify using direct bcrypt
        stored_hash = db_user['password'].encode('utf-8')
        if not bcrypt_lib.checkpw(pre_hashed_pw.encode('utf-8'), stored_hash):
...
async def sign_up(user: SignUpRequest):
    ...
    # Pre-hash with SHA256, then use bcrypt directly
    pre_hashed = hashlib.sha256(user.password.encode('utf-8')).hexdigest()
    # Convert to bytes for bcrypt
    hashed_password = bcrypt_lib.hashpw(pre_hashed.encode('utf-8'), bcrypt_lib.gensalt())
    # Store as string
    hashed_password_str = hashed_password.decode('utf-8')
    
    # Save the user to the database
    user_data = {"Uid": str(ObjectId()), "userName": user.userName, "email": user.email, "password": hashed_password_str, "type": "employee"}
...
async def update_credential(uid: str, credentials: Credentails):
    try:
        ...
        # Pre-hash with SHA256
        pre_hashed = hashlib.sha256(credentials.password.encode('utf-8')).hexdigest()
